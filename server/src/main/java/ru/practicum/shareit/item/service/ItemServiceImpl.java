...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemDto> getAll(Long ownerId) {
        List<ItemDto> itemsDto = itemMapper.mapItemsToDto(itemRepository.findAllByOwnerId(ownerId));
        ...
        List<BookingForItemDto> bookingsDto = bookingMapper.mapBookingsToBookingForItemsDto(bookings);
    ...
    @Transactional
    @Override
    public ItemDto create(ItemDtoPost itemDtoPost, Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundException("Нельзя создать вещь. Пользователь не найден id: " + userId));
        Item item = itemMapper.itemDtoPostToItemModel(itemDtoPost);
        item.setOwner(user);
    ...
    @Transactional
    @Override
    public ItemDto update(ItemDtoPost itemDto, Long id, Long userId) {
        Item item = itemRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Вещь не найдена id: " + id));
    ...
    @Transactional
    @Override
    public CommentDto createComment(Long itemId, Long userId, CommentDtoPost commentDto) {
        User user = userRepository.findById(userId)
